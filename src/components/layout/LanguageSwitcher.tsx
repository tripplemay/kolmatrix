  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        disabled={isPending}
        className={cn(
          "hover:text-brand-400 hover:bg-navy-700/60 text-on-surface-variant inline-flex h-9 items-center gap-1 rounded-full px-2.5 text-[12px] font-semibold tracking-wide transition-colors",
      ...
      {open ? (
        <ul
          role="menu"
          className="bg-navy-800 ring-brand-500/20 absolute right-0 z-[60] mt-2 w-44 overflow-hidden rounded-[14px] p-1 shadow-[0_12px_32px_rgba(0,0,0,0.45)] ring-1"
        >
          {LOCALES.map((l) => (
            <li key={l}>
              <button
                ...
                className={cn(
                  ...
                  l === locale
                    ...
                    : "text-on-surface hover:bg-navy-700/60 hover:text-brand-400"
                )}
              >
                {t(`locale.${l}`)}
              </button>
