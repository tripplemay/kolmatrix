...
**含交互 `'use client'` / SSR 页面 L2 必须 headless 真点（硬 gate，铁律级 —— 见 `harness-rules.md` 铁律 14）：**

验收含交互的 `'use client'` / SSR 页面时，L2 必须用 headless 浏览器**真点**关键控件并断言：
1. **console 无 React #418/#425 水合错误**（#418 = 文本节点失配会废掉整个 hydration root 交互，同 root 内所有控件事件处理器都来不及绑；机制详见 `framework/patterns/web-runtime-patterns.md` 客户端水合正确性节）
2. **点击产生预期 onClick 效果**（state 切换 / toast / 跳转）

**测法铁律：** 必须用标准 `locator.click()`（Playwright 自动等 actionability/enabled）**或**先 `await [data-ready=true]` 再点。**严禁 `force: true` / `dispatchEvent` / `evaluate(el => el.click())`** —— 这些跳过 enabled 检查，会点在水合时序窗口内未绑事件的按钮上，**稳定复现「假 bug」**（BL-108 Evaluator 两轮 reverify 即因落此窗口误判开关失效）。

**反例：** RTL `render()` 是纯客户端渲染从不经 SSR+hydrate，单测全绿 ≠ 真实浏览器无水合问题；含交互 SSR 页面不能只靠 jsdom 单测签收。

来源：BL-108-F004 fix-round 1（#418 水合失配）+ fix-round 2（force-click 落时序窗口误判）+ 用户 2026-06-10 ack。

**角色门禁 console 探针（feature 含角色限定路由 / next-intl ICU 模板时必做）：** L1 全绿 ≠ verifying PASS —— server console / pm2 runtime error（`FORMATTING_ERROR` / next-intl ICU 未绑占位符 / role enum mismatch）不改 HTTP 200/307 状态码，CI + audit script 全 PASS 也抓不到。手动跑角色门禁探针的完整方法（登录每角色账号直访角色限定路由 + 观察 server console runtime error）见 `framework/patterns/testing-env-patterns.md` §7「L1 全绿 ≠ verifying PASS：角色门禁 console 探针」。来源：BL-065-R1。
...
## 16. 数字层 verifying gate criterion — 语义 vs 字面自检

**背景：** verifying gate 的量化 criterion 必须**锚定语义信号**而非**字面数字**。Acceptance 文字写「快」/「少」/「几乎无」等定性词时，Evaluator 必须要求 Planner 修订为可量化 threshold；同时**预判 dataset 真实形态**避免落入「字面陷阱」。这是 stack-agnostic 的验收设计口径（非技术栈 env pattern），故留在本文件流程节而非迁至 patterns/。

**字面陷阱反面案例：** BL-066 F007 spec 写「top-15 内 max-min ≥ 5」criterion，staging recompute 后 top-15 全 clamp 100，criterion 字面失败但「mega-nano 不再同为 100」的**语义**已达成。正解是 data-driven 修订 criterion 为 (a') 全 dataset spread + (b') top-15 最小 follower threshold，而非调 formula。
...
- [ ] 数字层 criterion 必伴 fallback：若字面 fail，是否能改 data-driven 修订 criterion（无需 fix formula）

**应用：** 所有数字层 acceptance criterion（perf / size / coverage / 量化质量）验收前必经「语义 vs 字面」自检 + dataset 形态预跑。
