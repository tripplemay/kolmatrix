...
test.describe("BM1 — full marketer journey (F009 E2E)", () => {
  ...
  test.skip("login → product → discover → save → database → profile → dashboard", async ({ page }) => {
    ...
    await modal.locator('textarea[name="targetAudience"]').fill("APAC mobile gamers aged 18-30");
    await modal
      .locator('textarea[name="uniqueSellingPoints"]')
      .fill("Cross-platform MOBA with deep progression.");
    const generateCheckbox = modal.locator('input[name="generateImmediately"]');
    ...
    const unsavedCard = page
      ...
      .filter({
        has: page.locator('[data-testid="kol-save-button"][aria-pressed="false"]'),
      })
      .first();
    ...
    const pinnedCard = page.locator(`[data-testid="kol-card"][data-kol-id="${savedKolId}"]`);
