...
type: project
---
## 🔄 BL-060-soft-delete-ui-filter-hotfix REVERIFYING（5/9 19:01 / fix-round 1 staging reverify PARTIAL @ cc82a54 / CI 25597635544 全绿 / staging deployed git_sha=cc82a54 healthy）— 修 dashboard kolCount 2889→305 + database total 4→0 + kpi-snapshot trend cron + 4 文件 P1 一致性 + SQL ops 清 4 个 is_saved leak / spec docs/specs/BL-060-soft-delete-ui-filter-hotfix-spec.md / 视觉 baseline regen @ 6f30c08 / Reviewer：L1 全绿、database-fidelity 前 3 条 PASS，但 Bulk Action Bar / header CTAs 两条仍在 login() waitForURL(/dashboard/) 超时，暂不签收
...
## 🆕 BL-061-apify-fork-totallikes-verify backlog **BLOCKED**（high P1，~3-4h；2026-05-09 19:40 用户告知 fork 修复版未 deploy 到 /opt/apify-kol-service，整批等爬虫团队完成 fork §9 上线 ETA 待确认）— F001 fork deploy 推动+确认 / F002 staging 4 平台实物 / F003 staging daily sync ≥95% / F004 UI tooltip / F005 prod redeploy + 24h 监控
...
## 用户手工待办（按优先级）
1. **推动爬虫团队 deploy fork 修复版到 /opt/apify-kol-service**（BL-061 阻塞解除前置；含 staging 集成测试 + 通知 KOLMatrix）；其他 5 fork bug + revoke classic PAT 仍在
