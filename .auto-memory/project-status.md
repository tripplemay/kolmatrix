...
---
## 当前批次
- **BI4-architectural-guardrails** — status=**verifying**，5/5 features 完成（2026-04-23）
  - F001 Job Queue / F002 event_log / F003 audit helper (沿用 B0 AuditLog) / F004 Cursor pagination / F005 KOL tsvector
  - pre-impl 审计 `docs/specs/BI4-f003-f005-preimpl-audit.md` 解决 F003/F005 spec 漂移
  - 新增 29 测试（6+4+5+8+6），全绿；Reviewer (Codex) 待验收
...
BAux1 ✅ → **BI4 架构护栏** ✅ building 完成 → BM1 控制台+KOL核心(~9f, 7-10 天) → BM2 Campaign+联系+CRM+ROI+周报(~13f, 10-14 天) → MVP 上线 → 种子用户反馈
...
- BI4 pre-impl 审计 → Planner 裁决：F003 沿用 B0 audit_log 表（不新建 migration）/ F005 tsvector 映射 display_name+handle+categories+bio（非 spec 字面的 name+tags+knownBrandCollabs）

## Framework
v0.9.2（BI2 DB 命名 + PM2 zero-downtime 3 条件）；BAux1 round 3 + BI4 building 暂未沉淀新通用规律
