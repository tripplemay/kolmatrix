...
- **F003 backfill 实跑**: 342 积压候选/records 原空表(实证从没跑过)。120s 期 ~62% 成功且 ~50% poll-timeout(yield 瓶颈)→ rebuild 上 F005 300s 后**重跑全部剩余 235**。当前 **YT 邮箱覆盖 184→291(+107 解锁)**, 235 在 300s 下 drain 中
- ⏳ 剩余: F003 300s drain 完回填最终统计 → 转 verifying 交 Codex F004。**F001 runtime 触发验证 blocked**(需 refresh 命中 hasBusinessEmail 跃迁, 但 TikHub 余额耗尽 refresh 不跑 → 待充值/BL-092; 单测已覆盖逻辑)
