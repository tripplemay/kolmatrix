...
type: project
---
## 🚧 BL-108-crawler-pause-switches REVERIFYING (4/4 generator done, fix-round 2 修水合时序竞态) — 爬虫暂停开关
...
- ✅ **fix-round 2 (706d806, staging@706d806)**: reverify 真根因=**水合时序竞态**(开关 DOM 728ms 出现但 onClick 1253ms 才绑定, 中间窗口点击被永久丢弃且 React 事件重放不补触发; 真实用户加载后~1s 内点击同样失效)。修复=水合门闸(useSyncExternalStore: 水合前开关 disabled+data-ready=false+'初始化中', 水合后 enabled)。staging 实测 PASS(标准 click 自动等 enabled→弹窗成功, console0错)。⏭️ 待 Codex reverify(**用标准 click 或 await data-ready=true, 勿 force click**)
