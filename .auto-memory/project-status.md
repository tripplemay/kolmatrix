...
- **PR #6(Bug A)/#7(Bug B)/#8(F003)已全 merge → master f1d1bb7**(用户授权直接 merge)。合并后联合套件 17f/117t 绿
...
- ⏳ **剩余 gate**(未转 verifying): (1) **/opt rebuild**(git pull master + docker compose up -d --build, OOM 谨慎)才让 Bug A/B 在 prod 生效; (2) F003 drain 完成回填统计。kolmatrix 侧零运行时改动→staging 豁免
...
2. **/opt rebuild**(PR #6/#7/#8 已 merge master f1d1bb7) → git pull + docker compose up -d --build(OOM 谨慎)让 Bug A/B 生效
