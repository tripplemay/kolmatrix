...
## 🆕 BL-061-apify-fork-totallikes-verify backlog ready（high P1，~3-4h，等 BL-060 done 启动）— F001 SSH fork deploy 验证 / F002 staging 4 平台实物 / F003 staging daily sync engagement_rate ≥95% 非 NULL / F004 UI tooltip 'YT/X is view-based proxy' / F005 prod redeploy + 24h 监控 + BL-058 关闭
...
## 用户手工待办（按优先级）
1. ~~反馈爬虫团队 totalLikes~~ ✅ 已完成 5/9 19:30；其他 5 fork bug + revoke classic PAT 仍在
2. 5/15 §4.8 seed_expansion（如 cron 累积 ≥1k）+ fork 数据 4 维度迭代（BL-058 整体仍 low）
3. 决定下一批次方向（BL-061 验证优先 / BL-054 / BL-056 / 用户提其他）
## 关键决议（已 lock）
