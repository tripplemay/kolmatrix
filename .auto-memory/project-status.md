...
type: project
---
## 🔨 BL-070-reach-insight-cleanup BUILDING（3/8, fix_rounds=0, spec=0947d58, role_assignments=null 默认映射, 项目近期最后一批）
...
- ✅ F003 done: /insight 实装 (real route + 3 tab: dashboard/reports/analytics 默认 dashboard 渲染 DashboardPage 嵌入) + git mv /weekly-report → /insight/weekly-report (16 files R rename) + 4 条 301 (dashboard/reports/analytics → tab + weekly-report wildcard) + 全 callsite /dashboard → /insight 同步 (middleware.ts root+login redirect / login/actions / apify-preview / WeeklyReportHeader / RoiHeader / SidebarNav / PageTitleClient fallback) + 9 InsightTabs 单测 + 4 middleware 301 测试 + 2 visual baseline 暂 skip (dashboard.png + en-network-status-online.png 留 F007 regen)
- → F004 5 老路由目录 git rm + middleware redirect 删 (4h) → F005 6 BL-066 unmount 组件 + i18n deprecated keys 全删 (4h) → F006 4 路由 IA e2e suite 全量重写 (16h) → F007 视觉 baseline 全量 regen + reach/insight i18n 5 locale (6h) → F008 staging + prod deploy + 24h 监控 + 对外上线 12 项 checklist signoff (8h)
