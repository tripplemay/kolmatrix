...
- 主分支 HEAD: 02708c0；CI 8/8 jobs success on 02708c0；staging git_sha 仍 c302eb4（visual scaffold 不需要 redeploy）
- Asset 表 staging 实测 10 行 = email_template 10 行（dual-write parity）；RLS 启用 + policy `asset_tenant_isolation`
- Soft-watch 状态：S3 部分关 ✅ — visual baseline 2/5 入库（en-assets.png + en-assets-wizard-step1.png 由 Reviewer 触发 update-visual-baselines workflow run 25270711426 产出 + EXPECTED_BASELINES 同步至 17，CI 验证通过）；3 项 deferred 到 BL-025-followup mini-batch（empty-state / wizard-step3 / detail-as-modal-mobile，都需源码先加 testid 或先 fix S2）
- 余 5 条 Soft-watch 不阻塞：S1 F003 测试位置 / S2 <1280px modal 后备 / S4 F009 Pattern 6+7 / S5 wizard discard 多花 1 次 AI 调用 / S6 prod 仍 a3b0cd1 等用户 SSH redeploy
...
- Backlog 18 条：BL-025-followup（visual 3 PNG + S2 modal）/ BL-020 high / BL-021 medium / BL-023 medium / BL-024 medium / BL-026 deferred / BL-027 low / 余 10 条
