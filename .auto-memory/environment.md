...
- **Actions 清单：**
  ...
  - **BL-084-F002 (2026-06-05 创建)：** `kol-match-rerank` (action_id `cmq0hrq25016kbnpe2oru2qb0`, model claude-haiku-4.5, 用于 AI Match Panel getCampaignSuggestions 对 cosine 召回 top-30 KOL 候选按 campaign meta 重排 + 每条 ≤15 词英文 match reason；input candidates_json + campaign_name/markets/target_audience/budget，output `{ranked:[{kolId,rank,reason}]}` strict permutation；含 `<USER_*>` XML wrapper guard + 5 项 FINAL SELF-CHECK；3 候选实测 input=1439 / output=159 token，30 候选预估 ~$0.011/call) — env var `AIGCGATEWAY_MATCH_RERANK_ACTION_ID` 已 SSH 落入 **staging**（2026-06-05 06:13 UTC，backup `.env.staging.bl084-f002.20260605-061308`，pm2 reload --update-env，/proc/$PID/environ 实测含新 var，health 200 git_sha a2fd5ba）；**prod 待用户部署时注入**
  - **⚠️ staging build OOM 经验（2026-06-05 BL-084）：** `npm run build` 在 staging VM 默认 Node heap 下 SIGABRT（V8 old-space OOM，非系统 RAM——free 4.6Gi 仍崩）。修复：build 前 `export NODE_OPTIONS="--max-old-space-size=4096"`。OOM 后 .next 不完整 → pm2 reload 起 502；必须重 build 成功再 reload。staging deploy 标准步骤加此 export。
...
- **TIKHUB_TOKEN：** 由爬虫团队提供，存 `/opt/apify-kol-service/.env`，KOLMatrix 不读。**2026-06-05 ops 教训：** 爬虫团队给的"新 TIKHUB token" 实测 invalid（TikHub API 返 "Invalid API token"），rollback 还原 pre-ops 旧 token 恢复正常。**配置前必须先 dry-run** — TikHub 暂无公开 token 验证 endpoint，建议改 fork .env 前先用旧 token 跑一次基线 grep `Invalid API token` 计数，配置后 15s 内再 grep 对比。备份：`/opt/apify-kol-service/.env.bak.bl083-ops.20260605-022607`（pre-ops 旧 token）
- **APIFY_API_TOKEN：** Apify 平台 token，给 fork 端 5/8 接的 YT business email actor (`dataovercoffee/Youtube-Channel-Business-Email-Scraper`) 用。存 `/opt/apify-kol-service/.env` 2026-06-05 起，KOLMatrix 不读。**配置前 dry-run 模式：** `curl -H "Authorization: Bearer <token>" https://api.apify.com/v2/users/me` → 200 即 valid，401 即 invalid（推荐每次轮换前用此 endpoint 验证）。备份：`/opt/apify-kol-service/.env.bak.bl083-apify-only.20260605-023326`
