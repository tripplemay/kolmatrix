 ...
 *
 * Why a sidecar file:
 *   The exclude array grew past 80 lines as B5/B7a/BIx-F004 landed
 *   DB-touching helpers covered by integration specs. Keeping the
 *   rationale comments inline in vitest.config.ts crowded out the
 *   coverage thresholds + reporter wiring; lifting the array out
 *   keeps the main config skimmable while preserving every "why
 *   this entry exists" note next to its path.
 *
 * Edit policy:
 *   Each entry MUST include the rationale (which integration test
 *   covers it) inline. Remove an entry only when its integration
 *   coverage is removed or the file becomes pure (no Prisma /
 *   googleapis / aigcgateway calls). When adding a new exclusion,
 *   land it together with the integration test that authoritatively
 *   covers the helper, so the exclusion does not silently mask a
 *   coverage gap.
 */
export const COVERAGE_EXCLUSIONS_RUNTIME = [
  ...
  // to-end by tests/unit/kol-sync-refresh-selector.test.ts.
