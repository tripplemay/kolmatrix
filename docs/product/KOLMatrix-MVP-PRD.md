...
- ✅ **YouTube Data API 持续 / 自动 sync 现已允许**（用户 2026-04-27 二次决议；原 MVP 锁定的 ❌ 已解锁）
  - **B6-kol-daily-sync** KOLMatrix 自建每日 cron 增量同步（30-50 KOL/day，~2026-05-12 ~ 05-17 落地，不依赖外部团队 — 核心理由：自主可控数据增长）
  - **BL-012 爬虫团队** 6 月接入（M4 ~2026-06-15 + M5 ~2026-06-25），交付 5K KOL + 持续 sync
  - **共存策略**：B6 + BL-012 双数据源，按 metadata.source 字段区分（`youtube-api-daily` / `crawler-team`），互为容灾
  - 一次性 seed `docs/specs/MVP-kol-seed-redo-spec.md`（已建 1000+ 真高粉 demo seed）+ B6 接力持续增长
  - B6 详见 `docs/specs/B6-kol-daily-sync-spec.md`
- ❌ 邮件模板编辑器（B4）
- ❌ 邮件队列监控页（B4）
- ❌ 退订管理自动化（B4）
- ❌ 客户协同筛选页（B7）
