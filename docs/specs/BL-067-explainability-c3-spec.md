...
> **预估工时：** 6 day Generator + 1 day Reviewer（F001 0.5d + F002 1d + F003 0.5d + F004 1d + F005 1.5d + F006 1d + F007 0.5d = 6d；不含 24h cost 监控等待期）
...
- F001 commit 含 prompt 原文（中文系统提示 + 输入 schema 文档化）
- L1 PASS（无代码改动，仅 action 注册 + env 文档化；如有 prompt 单测则同 commit 加）
- staging git_sha 与本 commit 一致

---

### F002 — asset 缓存层 + cost cap 复用
...
- Migration: `prisma/migrations/20260516XXXXXX_bl067_f002_explanation_asset_types/migration.sql`
  - 加 enum 值 `ai_recommendation_explanation_short` / `ai_recommendation_explanation_detailed` 到 `AssetType` enum
...
- cost cap 复用：导入 `src/lib/cost-cap/check.ts`（BL-034 F005 中央策略）`checkLlmCostCap(tenantId): { allowed: boolean; remainingUsd: number }`；BL-067 不新增 cap 计算代码
- Cleanup cron：调研现有 asset cleanup 机制（如已有 → 沿用；如无 → F002 范围加 `scripts/cleanup-expired-explanation-assets.ts` per-day cron 删 24h 过期行）
...
- 新 worker `src/lib/queue/explain-recommendations-worker.ts` 处理 job type `recommendation-explain-prewarm`：
  ...
  - jobId 幂等：`prewarm-{tenantId}-{campaignId}`（同 jobId 已 active/waiting/delayed 则 noop；走 BullMQ `addJob` 的 `jobId` + `removeOnComplete: true`）
  - Worker 主循环：
    1. for each kolId in kolIds（按顺序）：
    2. `checkLlmCostCap(tenantId)` → 满 → `break`（剩余 KOL 不再调 LLM；不记 audit warning per BL-034 风格）
    3. 查 `readShortExplanation` per locale 全 5 locale 命中则 skip 该 KOL（避免重复调用）
    4. 拼 input：`{ kol: { id, name, followerCount, engagementRate, categories[] }, campaign: { id, name, markets[], productId, product: { name, targetAudience } }, valueScoreBreakdown: { followerScore, engagementScore, categoryScore, total }, locales: ['en','zh','ja','ko','es'] }`
    5. `aigcgatewayClient.runAction(AIGCGATEWAY_EXPLAIN_SHORT_ACTION_ID, input)` → 解析 5 locale JSON
    6. for each locale → `writeShortExplanation(tenantId, campaignId, kolId, locale, content[locale])`
    7. `logAudit({ action: 'ai_recommendation.explain_short_generated', ..., after: { kolId, locales: 5, tokenUsage, costUsd } })`
  - Worker concurrency=2（保守，避免 LLM rate limit）+ retry=1（仅网络错误 retry，JSON 解析失败不 retry）
- 新文件 `src/app/[locale]/(app)/campaigns/[id]/prewarm-actions.ts`（"use server"）含：
  - `enqueueExplanationPrewarmAction({ campaignId, kolIds }): Promise<void>` — server 端 enqueue
- AiRecommendationPanel（F003 改造的延续）mount + smart-match 返回 top30 后调 `enqueueExplanationPrewarmAction({ campaignId, kolIds: top30.map(k => k.id) })`
  - server 端 enqueue 幂等：同 jobId 已存在 → noop（per BullMQ `jobId` 行为）
  - 用户进出页面多次不会触发重复 LLM 调用
- 单测 ≥5 case：success / cap 满静默 break / LLM error skip 当前 KOL / 幂等 jobId / 空 kolIds noop
- L1 PASS
- staging git_sha 与本 commit 一致
...
1. **cost cap 复用 BL-034 F005**：F002 + F004 + F005 全部从 `src/lib/cost-cap/check.ts` 导入 `checkLlmCostCap`，不新增 cap 计算代码；与 outreach AI 邮件个性化共享同一 $5/day/tenant 配额
...
10. **prewarm jobId 幂等**：用户来回切 detail 页不会触发重复 LLM 调用；同 campaign 多次 mount → 同一 BullMQ job
...
| cost 估算偏低（实际 token > $0.0015/call） | 中 | 中 | F007 24h cost 监控 → 超 $5 cap 即 silent fallback；BL-034 F005 cap 中央策略已 proven |
| BullMQ 队列堆积（并发 campaign 创建多） | 低 | 低 | jobId 幂等防重 + worker concurrency=2（保守，避免 LLM rate limit） |
| asset 表膨胀（24h TTL 后未清理） | 低 | 低 | F002 调研现有 cleanup 机制；如无则同 commit 加 per-day cron |
