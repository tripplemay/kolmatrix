...
## 3. 抓取慢：根因是 TikHub 凭据/账户问题 + 预算配比失衡

> 🔴 **根因修正(2026-06-06 重查 — 用户称账户有钱后复查):** 下文 §3.2"余额耗尽"的**症状描述准确**(scrape 自 6/04 归零、日志 95% Insufficient balance),但**根因不是"忘充值",而是凭据/账户层面**:
> - 爬虫 `.env` 当前**有效** token(`i5kiE/…rBCQ==`)所属 TikHub 账户 = `71***@qq.com`,直查 `get_user_info` 实测 **balance=$0.0005 / free_credit=0**(空)。
> - 用户记忆/手贴的 token `**y**i5kiE/…rBCQ==`(多开头一个 `y`)直查返回 **401 Invalid API token**(无效,系抄录笔误)。
> - 即:**有效的 token 指向空账户,用户以为充值的账户与 token 不匹配(或充值未落到 71@qq.com)。** 修法 = 确认充值账户 + 用其正确 API key 换 `.env`,而非单纯充值。
> - 安全:有效 token 在 402 调试响应中被回显、片段入对话 → 修复后建议 TikHub 后台轮换。
> - 状态(2026-06-06):等用户充值/提供正确 key 后,Planner 先验证账户+余额再换。下文 §3.2-§3.4 的配比分析(refresh 占 90%)独立成立、不受此修正影响。
