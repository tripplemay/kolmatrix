...
## 3. 抓取慢：根因是 TikHub 账户未充值 + 预算配比失衡

> ✅ **根因确认(2026-06-06,用户 + 爬虫团队核实):** 就是**账户没充值**——§3.2 的"余额耗尽"判断正确。爬虫 `.env` token **有效**、所属账户 `71***@qq.com` **正确**(实测当时 balance=$0.0005 / free_credit=0,空),用户正往该账户充值,充值后即恢复。
> - 一段插曲:复查时用户手贴的 token `**y**i5kiE/…rBCQ==`(多开头一个 `y`)直查返回 **401 Invalid**,一度疑似"账户不匹配";经核实那只是**记忆抄录笔误**,与部署中的有效值(`i5kiE/…`)无关。教训:症状(空余额)早已确认,不应被一个手误 token 带偏成"凭据不匹配"假设——但查证(直调 get_user_info)是对的。
> - 安全:有效 token 在 402 调试响应中被回显、片段入对话 → 修复后建议 TikHub 后台轮换。
> - 下文 §3.2-§3.4 的配比分析(refresh 占 90%)独立成立、与充值无关,仍是积累期该做的优化(BL-086)。
