...
| 档位 | 用途 | 推荐模型 | 参数 | 估算 cost（1000 次调用） |
|---|---|---|---|---|
| **批量档（L1）** | 日常 KOL crawler 入库评分、粗筛 | `deepseek-v3` | max_tokens 1024 | ~$1 |
| **精评档（L2）** | 客户候选名单 top 50 精评、品牌安全审查 | `claude-sonnet-4` / `gpt-4o` | max_tokens 2048 + response_format json | ~$20 |
| **匹配档（L3）** | Campaign × KOL 匹配打分、个性化邮件生成 | `gemini-2.5-pro` / `deepseek-v3` | max_tokens 1024 | ~$5 |

**选择逻辑（在代码中）：**

```typescript
// src/features/kol-eval/model-selector.ts
export function selectModel(purpose: 'bulk_eval' | 'precision_eval' | 'campaign_match'): string {
  const strategy = {
    bulk_eval: 'deepseek-v3',
    precision_eval: 'claude-sonnet-4',
    campaign_match: 'gemini-2.5-pro',
  };
...
### 4. Prompt 管理：用 aigcgateway Action（而不是自管）

**决策：** Prompt 模板在 aigcgateway 控制台创建 Action，KOLMatrix 按 Action ID 调用。

**好处：**
- Prompt 版本化（v1 / v2 / ...），不用重新 deploy KOLMatrix 即可切换
- 变量化（`{{ kol.handle }}`, `{{ kol.bio }}` 等）
- Action 日志集中在 aigcgateway audit 里（跨项目统一）
...
const result = await gw.runAction({
  actionId: 'kol-eval-v1',  // aigcgateway 控制台创建
  variables: {
...
**Prompt 模板初版由 Planner（Kimi）起草，用户 review 迭代。** B2 spec 阶段启动此工作。
...
估算分配（B2 启动时）：
- KOL 批量评分（L1 DeepSeek）：$20 / 月（~20,000 次调用）
- KOL 精评（L2 Claude）：$30 / 月（~1,500 次调用）
- Campaign 匹配（L3）：$20 / 月（~4,000 次调用）
- B4 邮件个性化（未来）：$30 / 月（~1,500 封）

**超支防线：**
- aigcgateway 预充值机制 → 账户余额用完自动停止调用（硬限）
- KOLMatrix `ai_call_log` 表记录每次调用 cost → 月度聚合可告警（软限）
- 初期每月末人工核查 → 稳定后加 cron 自动告警

### 6. 审计与追溯

每次 AI 调用写 `ai_call_log` 表（待 B2 migration 建）：
...
B2 启动前（BI3 完成后），Planner 起草以下 prompt 模板候选给用户 review：

1. **KOL 批量评分**（L1 DeepSeek）：从 KOL bio + 粉丝数 + 内容类型 → 输出 0-100 分 + 4 维 breakdown
2. **KOL 精评**（L2 Claude）：从完整 KOL 画像 → 输出详细评分 + 文字说明 + 风险提示
3. **Campaign × KOL 匹配**（L3）：输入 campaign 定位 + KOL 画像 → 输出匹配度 0-100 + 推荐理由
4. **邮件个性化**（L2 Claude，B4 用）：输入 KOL 画像 + campaign 描述 → 输出个性化邮件主题 + 正文

每个模板 3-5 个版本候选，用户选定或 merge 后在 aigcgateway 控制台建 Action。
