...
| [ui-fidelity-guardrail.md](ui-fidelity-guardrail.md) | 任何有设计稿原型参照的 UI 页面 feature（spec 4 段硬要求 + 还原度验收）；**landing / marketing 视觉精修（token layer 规范 + 现代 CSS 渐进增强 / motion 三层守门）** | 三角色 |
...
| [web-runtime-patterns.md](web-runtime-patterns.md) | 引入 alpha/beta/rc 依赖；反向代理后构造对外绝对 URL；**异步/后台任务队列（fire-and-forget / prewarm / BullMQ 升级）；含交互 client/SSR 页面水合（#418 / mount-gate）；新建或改 App Router route segment 或 `use server` 文件；大型删除/重构批次；顶层 IA 重构 / 路由迁移 / redirect middleware；next/image 异构 CDN；Suspense fallback / CLS** | Generator |
| [testing-env-patterns.md](testing-env-patterns.md) | L1/L2 验收命中 Prisma / Node 版本 / jsdom / Playwright E2E / 字体子集 / RLS 查询；**角色限定路由 console 探针、motion a11y 三件套、server-action mock 不可行、chaos/edge-case staging-flag、IA refactor advisory test + STRICT_MODE、base-ui/next-intl E2E 断言陷阱**（见文件底部触发速查表） | Evaluator |
